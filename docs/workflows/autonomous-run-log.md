...
- **Shared:** `publicTicketAiIntakeResultSchema` and `PublicTicketAiIntakeResult` accept `'openai'` and `'stub'` providers. New `apps/api/src/modules/public/ai-cost-guard.ts` exports `readAiBudgetConfig`, `decideAiBudget`, `estimateCostMicros`, `extractOpenAiUsage`, `totalTokens`. The cost guard is pure (no Prisma) for easy testing.
...
- **Verification run:** `PRISMA_GENERATE_NO_ENGINE=true pnpm db:generate`; `pnpm typecheck`; `pnpm --filter @kentos/api test` (11 new ai-cost-guard tests pass alongside existing citizen identity reconciliation and attachments tests); `pnpm build`. **Not run by design:** real OpenAI API calls — they are paid; the budget guard and telemetry tests cover the deterministic logic against pure helpers and stub responses.
- **Result:** Passed. Schema regen, typecheck, all api tests, and full workspace build are green. Provider order is `openai > stub`; budget guard forces stub fallback when daily token or cost budget is reached; telemetry rows include latency/tokens/cost/success and tolerate DB failures without breaking ticket intake.
- **Next action:** Operator to (1) populate `OPENAI_API_KEY` in `.env.production.local` plus a daily token budget, (2) review weekly cost via `AiRun` aggregates, and (3) decide if cost rates need adjustment for tenant-specific contracts.
...
- **Blocker:** Live EMAIL outbound needs `EMAIL_FROM_ADDRESS` plus Postmark or SMTP credentials. Paid AI needs `OPENAI_API_KEY` plus budget settings. Retention delete is intentionally still disabled because it deletes production records/objects.

## 2026-05-12 - AI provider activation checkpoint (SUPERSEDED)

> **SUPERSEDED:** The native AI provider wired here was later removed. The runtime
> is now **OpenAI-only** (`OPENAI_API_KEY` → `gpt-4o`, deterministic stub fallback).
> See the konuşan AI maskotu + OpenAI migration commit. This entry is retained only
> as a dated marker; no native-provider configuration remains in the codebase.
