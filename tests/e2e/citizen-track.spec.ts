...
test('citizen track accepts TK tokens and rejects legacy KNT values', async ({ page, request }) => {
  ...
  await page.waitForURL(/\/demo-belediye\/track\?error=missing$/);
  await expect(page.locator('#trackingToken-error')).toContainText('Takip kodu');
  ...
  await gotoCitizenTrack(page);
  await page.getByLabel('Takip kodu').fill('TK-123');
  await page.getByRole('button', { name: 'Durumu sorgula' }).click();
  await page.waitForURL(/\/demo-belediye\/track\?error=format$/);
  await expect(page.locator('#trackingToken-error')).toContainText('Kod');
  ...
  await gotoCitizenTrack(page);
  await page.getByLabel('Takip kodu').fill('TK-FFFFFFFFFFFFFFFF');
  await page.getByRole('button', { name: 'Durumu sorgula' }).click();
  await page.waitForURL(/\/demo-belediye\/ticket\/TK-FFFFFFFFFFFFFFFF$/);
  await expect(page.locator('.notice.error')).toContainText('Takip kodu');
});
