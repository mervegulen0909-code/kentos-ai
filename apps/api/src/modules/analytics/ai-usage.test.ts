...
const rows: AiRunGroupRow[] = [
  { provider: 'anthropic', runs: 10, successCount: 8, tokensTotal: 12_000, costMicros: 36_000, totalLatencyMs: 50_000 },
  { provider: 'netiva', runs: 5, successCount: 5, tokensTotal: 4_000, costMicros: 12_000, totalLatencyMs: 8_000 },
...
run('summarizeAiUsageByProvider returns rows sorted by tokensTotal desc', () => {
  const out = summarizeAiUsageByProvider(rows);
  assert.equal(out.length, 3);
  assert.equal(out[0].provider, 'anthropic');
  assert.equal(out[1].provider, 'netiva');
  assert.equal(out[2].provider, 'stub');
  assert.equal(out[0].failureCount, 2);
  assert.equal(out[0].successRate, 0.8);
});
