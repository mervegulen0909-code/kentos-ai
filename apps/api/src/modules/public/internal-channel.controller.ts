...
const hashKey = (s: string) => createHash('sha256').update(s).digest();
...
export class InternalChannelController {
  ...
  ingest(@Headers('x-kentos-internal-key') internalKey: string | undefined, @Body() dto: IngestChannelEnvelopeDto) {
    ...
    if (!internalKey || !timingSafeEqual(hashKey(internalKey), hashKey(expected))) throw new ForbiddenException('Internal kanal anahtari gecersiz.');
